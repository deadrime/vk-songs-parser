...
export default class VKParser {
  ...
  async parseUserSongs(id) {
    ...
    while (1) {
      ...
      let songs = await this.parseSongsFromPage(cheerio.load(page), id)
      if (!songs) {
  ...
  async downloadSongs(songs) {
    ...
    let downloadCount = 0
    await Promise.all(songs.map(async ({artist, title, mp3}) => {
      const options = {
        url: mp3,
        encoding: null
      }
      ...
      fs.writeFileSync(`${__dirname}/songs/${artist} - ${title}.mp3`, buffer)
    }))
...
parser.init()
  ...
  .then(songs => {
    parser.saveToJSON(songs)
    parser.downloadSongs(songs)
  })